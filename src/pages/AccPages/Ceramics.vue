        ...
        key.checkedtitle
            ...
            <el-form :inline="true" class="demo-form-inline">
                <el-card v-for="a in key.checkedoptions" v-show="a.checkedvalue" style="margin-bottom: 20px;">
                    ...
                    <p style="float: left;color: #79BBFF;">{{ a.CO2value }}</p>
                </el-card>
            </el-form>
            <el-form :inline="true" class="demo-form-inline" v-show="key.show3">
                <br>
                <el-form-item v-for="k in key.option" :label="k.optionTitle">
                    <div style="margin-bottom: 10px;font-weight: bolder;">{{ key.optionTitle }}</div>
                    <el-input v-model="k.optionValue" placeholder="请输入数字" clearable />
                </el-form-item>
                <br>
            </el-form>
...
const getCarbpnemissions = async (is_store: number) => {
    ...
    allkey.forEach(async item => {
        if (item.checkedoptions !== undefined) {
            ...
            if (allkey.indexOf(item) == 1 && item.option![0].optionValue !== '') {
                // console.log(item.option)
                await setIndustrialProcessCeramic({
                    linked_id: oneid,
                    is_store: is_store,
                    enterprise_id: enid,
                    raw_material_consumption: parseFloat(item.option![0].optionValue),
                    raw_material_utilization_rate: parseFloat(item.option![1].optionValue),
                    calcium_carbonate_content: parseFloat(item.option![2].optionValue),
                    magnesium_carbonate_content: parseFloat(item.option![3].optionValue),
                }).then(response => {
                    // 处理请求成功的响应
                    let message = "计算完毕"
                    if (is_store === 1) {
