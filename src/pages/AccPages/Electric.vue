    ...
    <div class="contentdiv" v-for="(key , index) in allkey">
     ...
     <!-- 查看默认值 -->
        <know-default-button v-if="!index"></know-default-button>
            ...
            <p style="float: left;margin-right: 10px;color: #79BBFF;" v-if="key.show2">CO₂排放量为</p>
            <p style="float: left;color: #79BBFF;" v-if="key.show2">{{ key.CO2value }}</p>
...
import { setFossilFuel, setDesulfurization, setElectricity, setInformation, uploadFile } from '@/apis/accountapi';
        ...
        if (item.checkedoptions !== undefined) {
            item.checkedoptions.forEach(async ele => {
                if (ele.checkedvalue == true) {
                    ...
                    if (allkey.indexOf(item) == 1) {
                        if (ele.option[0].optionValue !== '') {
                            await setDesulfurization({
                                linked_id: oneid,
                                is_store: is_store,
                                enterprise_id: enid,
                                kind: ele.checkedlabel,
                                desulfurizer_consumption: parseFloat(ele.option[0].optionValue),
                                emission_factor: ele.defalutdiwei
                            }).then(response => {
                                // 处理请求成功的响应
                                let message = "计算完毕"
                                if (is_store === 1) {
