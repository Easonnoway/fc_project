...
contract CarbonReport is ERC20, Ownable {
    ...
    function getUserInfo(
        ...
        return (
            users[_user].enterprise_type,
            users[_user].carbonAccount,
            users[_user].record
        );
    }
    ...
    // 创建用户
    function createUser(
        string memory _enterprise_type,
        address _carbonAccount,
        string memory record_json,
        uint256 initialCarbonCredits
    ) public onlyOwner {
        require(
            users[_carbonAccount].carbonAccount == address(0),
            "User already exists"
        );
